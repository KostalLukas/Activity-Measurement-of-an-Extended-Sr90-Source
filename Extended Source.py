...
np.savetxt('Data/extended.csv', data_out, header=hdr ,delimiter=',', newline='\n')
...
plt.ylabel('fraction of detected quanta $f$ ($unitless$)')
...
plt.xlabel('distance $r$ ($m$)')
plt.ylabel('adjusted fraction $f \: r^2$ ($m^2$)')
plt.rc('grid', linestyle=':', color='black', alpha=0.8)
plt.grid()
plt.legend()
