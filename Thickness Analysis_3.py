...
# official value for E_max of Sr90 source from Y90 decay
E_acc = 2.280
...
if choose_measurements == 'auto':
    ...
    plt.ylabel('reduced $\chi^2$ $(unitless)$')
    plt.rc('grid', linestyle=':', color='black', alpha=0.8)
    plt.grid()
...
# relative difference between the official and calcualted value
diff = np.abs(E_acc - E) / E_acc 
...
print()
print(f'h_1             = {itc_1*1e3:.4g} ± {itc_1_err*1e3:.4g} mm')
print(f'h_2             = {itc_2*1e3:.4g} ± {itc_2_err*1e3:.4g} mm')
print()
...
print()
print(f'E_accepted      = {E_acc:.4g} MeV')
print(f'% difference    = {diff * 100:.3g} %')
