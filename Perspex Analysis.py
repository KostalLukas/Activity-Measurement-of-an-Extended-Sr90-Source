...
Act = 2.168818654733823
Act_err_p = 0.08607655824905633
Act_err_m = 0.08367346842559101
...
for i in range(1, len(E_stop)):
    ...
    # gamma factor to account for relativistic effects on density
    gamma = E_stop[:i] * sc.e / (sc.m_e * sc.c**2) +1
    
    # calcualte the reciprocal of the stopping power and integrate
    n_int = 1 / (n_stop[:i] * rho * gamma)
    h_stop[i] = num_int(E_stop[:i], n_int)
    
    # repeat for upper and lower bounds on density from systeamtic error
    n_int_p = 1 / (n_stop[:i] * (rho + rho_err) * gamma)
    n_int_m = 1 / (n_stop[:i] * (rho - rho_err) * gamma)
...
print()
print(f'A_measured      = {Act:.4g} +{Act_err_p:.4g} -{Act_err_m:.4g} MBq')
...
# save the plot
plt.savefig('Plots/Perspex/stopping_power.png', dpi=300, bbox_inches='tight')
plt.show()
...
# save the plot
plt.savefig('Plots/Perspex/density_effec.png', dpi=300, bbox_inches='tight')
plt.show()
...
# save the plot
plt.savefig('Plots/Perspex/stopping_thickness.png', dpi=300, bbox_inches='tight')
plt.show()
...
# save the plot
plt.savefig('Plots/Perspex/energy_spectrum.png', dpi=300, bbox_inches='tight')
plt.show()
